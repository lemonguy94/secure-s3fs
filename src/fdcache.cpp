...
#include "curl.h"
#include "Encrypt/rc4_enc.c"
#include "Encrypt/rc4_skey.c"

using namespace std;

//------------------------------------------------
// MY CRYPT IMPLEMENTATION
//------------------------------------------------
...
int decodeAndEncode(int fd){
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        //Convert fd to cstr which contains data from fd
	        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		int fdlen = lseek(fd, 0, SEEK_END);
	        unsigned char* fdbuf;
	        fdbuf = (unsigned char*) calloc(fdlen, sizeof(char));
	        pread(fd, fdbuf, fdlen, 0);
	        string fd_str(reinterpret_cast<char*>(fdbuf), fdlen);
		char const* fd_cstr = fd_str.c_str();
	        RC4_KEY key;
	        int datalen = fd_str.size();
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        ...
	        RC4(&key, datalen, (const unsigned char*)fd_cstr, buffer);
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        //Write to file
	        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        string ret((char*)buffer, datalen);
		const char *outbuf = ret.c_str();
		pwrite(fd, outbuf, ret.length(), 0);
		ftruncate(fd, ret.length());
		return 0;}
	else{
		printf("Error: password file not found.  Running with default key.\n");
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                //Convert fd to cstr which contains data from fd
                //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                int fdlen = lseek(fd, 0, SEEK_END);
